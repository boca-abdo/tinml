<?php
  ...
  $amz_book = protect($_POST['amz_book']);
  try {
    $step1_stmt = $tinml_con->prepare("UPDATE `users` SET `ara_book`=?,`isl_book`=?,`art_book`=?,`sci_book`=?,`mth_book`=?,`fre_book`=?,`soc_book`=?,`amz_book`=? WHERE `id`=?");
    ...
    $step1_stmt->bindParam(8, $amz_book, PDO::PARAM_STR);
    $step1_stmt->bindParam(9, $log_id, PDO::PARAM_INT);
    $step1_stmt->execute();
    echo "done";
